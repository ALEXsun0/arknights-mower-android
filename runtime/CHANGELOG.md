Android 0.2.4 内置 Mower 更新（基于 alpha 2f566ee8（已含 #1038），额外整合未合并 PR #1037）

- 保留选人后的排序刷新与完整名单、顺序校验；生产房未翻页且校验正确时省略重复职业筛选，名单无法确认或已翻页时仍复位。宿舍与训练流程不变。
- 选人列表复位统一使用职业筛选切换：ALL → 其他职业 → ALL，指定职业先切 ALL 再切回原职业；恢复原侧栏开关，不再反向拖动。
- 生产房起始仅一名待找目标且无 Free 时，排序后先选当前页可见目标，最终仍验证完整名单与顺序；未命中才复位搜索。
- 普通名字识别仅转换两条名字横带，排序箭头只处理对应区域；保持原阈值、模型、坐标与等待。
- Android 推荐跑单前置延时 5 分钟、葛朗台缓冲时间 15 秒，不覆盖用户已保存设置。
- 地图拖动后从下一次截图重新定位房间，不再按请求位移预测点击位置；保留原有有界进房重试。
- 稳定页面在紧邻步骤间消费一次，仍采集新画面复核；输入和模式变化使旧观察失效。
- 完全相同的归一化名字像素复用匹配结果；每帧重新定位卡片，资源更新清理缓存。
- 中枢干员经换帧确认仍为零心情时，跳过不存在的耗尽倒计时；保留宿舍恢复、菲亚梅塔和训练计时。
- 保留单回所需的两轮暂撤、确认、恢复阵容和计时刷新；本次不减少宿舍排序操作。
- 每房间换班耗时统计区分截图获取、名字匹配、等待、输入、筛选复位与校验，不保存截图。
- 排序、翻页或名单暂未稳定时，交回排班原有有界恢复；用户停止仍立即生效。
- 进房连续失败时返回游戏首页、重新进入基建定位；复核最后一次点击结果，错误信息包含目标房间。
- 翻页逐页检查并保留两列重叠；首列裁切时不把后续卡片当作已选名单；名单与坐标稳定才判断选择结果。
- 修改排班后重新生成宿舍优先级（独立 PR #1038）；纳入5301e5e9，保存失败恢复原排班及优先级状态，重试仍正确通知前端。
