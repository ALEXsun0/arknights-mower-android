...
- 连续执行回拉的中间手势，末端仍读取实际页面确认，不以滑动计数判定归零。
- 完全相同的归一化名字像素复用匹配结果；每帧重新定位卡片，资源更新清理缓存。
- 新增每房间换班耗时统计，区分截图获取、名字匹配、等待、输入和重试。统计不保存截图。
- 排序、翻页或名单暂未稳定时，交回排班原有有界恢复，不再直接结束排班线程。用户停止仍立即生效。
- 进房连续失败时返回游戏首页、重新进入基建定位；复核最后一次点击结果，错误信息包含目标房间。
- 移除 sleep 后多余的截图缓存清理调用；实际截图为懒加载，新增排序前后状态诊断。
- 回拉使用屏内 1100 像素路径并复核位置；计数为零也实际确认；首列裁切时不读取后续卡片作为已选名单。
- 翻页逐页检查并保留两列重叠；名单与坐标同时稳定才判断选择结果；正确名单不重复排序校验。
- 同次等待内复用完全相同名字图像的匹配结果；修改排班后重新生成宿舍优先级（PR #1038）。
