...
def install_update(target, callback=None, session=None, source='github', mirror_token='', system='android', machine=None, channel='stable'):
    ...
    if source != 'github': raise updater.MaaUpdateError('Android 组件目前使用 MAA 官方 GitHub 更新源')
    ...
    return {'version':release.tag, 'source':'github', 'channel':channel, 'operation':'update', 'platform':'android', 'arch':'arm64',
...
def get_release(session=None, channel='stable'):
    ...
    endpoint = 'https://api.github.com/repos/MaaAssistantArknights/MaaAssistantArknights/releases'
    response = client.get(endpoint + ('/latest' if channel == 'stable' else '?per_page=20'), timeout=30)
    response.raise_for_status()
    payload = response.json()
    if channel != 'stable':
        payload = next((r for r in payload if r.get('prerelease') and not r.get('draft')), None)
        if payload is None: raise updater.MaaUpdateError('官方发布列表暂无公测版 Android 组件')
    result = updater.parse_release(payload, system='android', machine='arm64', channel=channel)
    if not result.runtime.sha256: raise updater.MaaUpdateError('官方组件缺少 SHA256 校验信息')
    return result
