...
object GameFpsMonitor {
    ...
    private fun createTaskSource(packageName: String): TaskFpsSource? {
        ...
        val taskId = runCatching {
            val am = com.aliothmoon.maameow.third.FakeContext.get().getSystemService(android.app.ActivityManager::class.java)
            @Suppress("DEPRECATION")
            val task = am.getRunningTasks(100).firstOrNull { it.topActivity?.packageName == packageName || it.baseActivity?.packageName == packageName }
            task?.taskId
        }.getOrNull() ?: run {
