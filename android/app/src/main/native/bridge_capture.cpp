...
static std::atomic<bool> g_stopping{true};
...
void ReleaseNativeCapturer(JNIEnv *env) {
    ...
    if (g_capturer) {
        ...
        // Finish in-flight callbacks before close takes the reader's internal lock.
        // Android 12 can otherwise wait for a callback blocked in AImage_delete.
        { std::lock_guard<std::mutex> lock(g_callbackMutex); }
        ShutdownPreview(env);
        if (g_capturer->reader) {
            AImageReader_delete(g_capturer->reader);
