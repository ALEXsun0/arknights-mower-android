...
class RemoteServiceImpl : RemoteService.Stub() {
    ...
    override fun mowerFrame(): ParcelFileDescriptor? {
        ...
        kotlin.concurrent.thread(name = "mower-frame") {
            try {
    ...
    override fun mowerKey(code: Int): Boolean {
        require(code in setOf(4, 66, 67, 3))
        val id = VirtualDisplayManager.getDisplayId()
        check(id > 0) { "Game display is not running" }
        return InputControlUtils.keyDown(code, id) && InputControlUtils.keyUp(code, id)
    }

    override fun mowerGame(packageName: String, launch: Boolean): Boolean {
        require(packageName in setOf("com.hypergryph.arknights", "com.hypergryph.arknights.bilibili"))
        val id = VirtualDisplayManager.getDisplayId()
        check(id > 0) { "Game display is not running" }
        if (launch) return ActivityUtils.startApp(packageName, id, false)
        com.aliothmoon.maameow.third.wrappers.ServiceManager.getActivityManager().forceStopPackage(packageName)
        return true
    }

    override fun mowerText(text: String): Boolean {
        require(text.length <= 256 && text.all { it.code in 32..126 }) { "Only ASCII game input is supported" }
        val id = VirtualDisplayManager.getDisplayId()
        check(id > 0)
        ...
        val manager = com.aliothmoon.maameow.third.wrappers.ServiceManager.getInputManager()
        return events.all {
            com.aliothmoon.maameow.third.wrappers.InputManager.setDisplayId(it, id) && manager.injectInputEvent(it, 2)
        }
    }
