...
- 截图保存时间由原生设置接管，Android 默认 0 小时，仅保留内存预览；非 0 显示频繁写入风险，桌面配置导入不会覆盖本机选择。
- 局域网连接支持固定端口与自定义 WebUI Token；留空保留自动模式，保存后重启服务生效，端口冲突时明确提示。
- 修复后台连接超时后卡在“正在启动”的问题；后台启动改为自有启动器直连 AIDL，支持 Shizuku/Sui 与可选 Root。
- 修复未启动 Mower 服务时录制/测试解锁提前退出：仅连接后台授权，Mower 运行中才先停止任务；服务切换期间阻止录制。
