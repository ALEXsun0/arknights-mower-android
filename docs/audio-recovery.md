...
用户主动点击「关闭静音并恢复声音」时，关闭自动静音，检查已安装官服与 B 服的包级和 UID 级 `PLAY_AUDIO`，将非 `allow` 的声音模式明确恢复为 `allow` 并再次读取确认。该操作不依赖历史记录，也不需要启动 Python 服务、排班或后台游戏；仍需 Shizuku / Root 授权。读取、修改或验证失败会显示错误，不报告恢复成功。只修改声音权限，不重置游戏的其他权限。
