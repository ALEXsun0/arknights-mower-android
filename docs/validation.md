...
- 修复包实机窗口采样：前台静置 8 秒、后台 10 秒、恢复前台 5 秒，三个区间的重复 Relayout 与亮屏标志提交均为 0；切后台有明确暂停记录，重新进入后恢复显示。冷启动 Activity 400ms，返回前台 138ms。未启动游戏任务；锁屏解锁结果待用户反馈，完整整机卡死根因仍有上述证据限制。
