...
- 源码同步到 alpha `eaa05aa1`，叠加尚未合并的 #1029，合并提交 `d8cbb41b`。最新三项上游修复的六组 Python 测试 71 项通过；配置/宿舍候补/排班编辑前端测试 21 项通过，生产 WebUI 构建及同源检查通过。
...
- 原生收尾入口改为原有 Mower 配置的三选项，WebUI 自动保存省略 Android 隐藏字段；其他三个平台仍提交原字段。最终 APK 的该入口、最新版 alpha 和热更新包尚待最后安装检查。
