...
- APK 为 374,130,195 字节，versionName 0.2.0 / versionCode 14；复用已验证的 Python 运行环境，无需重打运行包。三星 SM-G9880 无线 ADB 覆盖安装成功，已回读 versionCode 14 并冷启动首页。实机重新录制和单次回放的结果待用户完成，不能将回归测试等同实机解锁成功。
